...
import cv2 as cv
# from cv2.cv2 import HoughLines, cvtColor, HoughLinesP, COLOR_RGBA2GRAY, COLOR_RGB2HSV, Canny
...
def Non_Local_Means(imagem):
    ...
    limpa = restoration.denoise_nl_means(imagem, h=0.6 * sigmaEstimado, sigma=sigmaEstimado, patch_size=5,
                                         patch_distance=6, multichannel=True, preserve_range=True)
    return limpa
...
def line_detection(img, t, sigma=0.3):
    v_component = cv2.cvtColor(img[:, :, :3], cv2.COLOR_RGB2HSV)[2]

    median = np.median(v_component)
    minimo = int(max(0, (1 - sigma) * median))
    maximo = int(min(255, (1 + sigma) * median))

    border = cv2.cvtColor(img, cv2.COLOR_RGBA2GRAY)

    border = cv2.Canny(border, minimo, maximo, None, 3)

    linhas = cv2.HoughLines(border, 1, np.pi / 180, t)
...
def probabilistic_line_detection(img, t, min_line_size, max_line_gap, sigma=0.3):
    ...
    border = cv2.Canny(img, minimo, maximo, None, 3)

    linhas = cv2.HoughLinesP(border, 1, np.pi / 180, t, None, min_line_size, max_line_gap)
    ...
    for linha in linhas:
        ...
        cv2.line(result, (x1, y2), (x2, y2), (255, 0, 0), 3, cv2.LINE_AA)
...
def circle_detection(img, min_radius, max_radius):
    ...
    blur = cv2.cvtColor(blur, cv2.COLOR_RGBA2GRAY)
    ...
    for circulo in circulos[0]:
        ...
        cv2.circle(result, centro, circulo[2], (255, 0, 0), 2)
        cv2.circle(result, centro, 2, (0, 0, 255), 3)
